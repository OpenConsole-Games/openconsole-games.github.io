...
GameLoad.prototype.initialize = function (gameLocation, canvasId) {
  gLoad.gameDir = gameLocation.substring(0, gameLocation.lastIndexOf("/") + 1);
  gLoad.gameLocation = gameLocation;
  gLoad.canvasId = canvasId;
...
GameLoad.prototype.iframeCanvasSize = function () {
  if (gLoad.canvasId == null || gLoad.gameIFrame.contentWindow.document.getElementById(gLoad.canvasId) == null)
    gLoad.canvasElems = gLoad.gameIFrame.contentWindow.document.getElementsByTagName('canvas');
  else
  ...
  for (var i = 0; i < gLoad.canvasElems.length; i++) {
    var canvasElem = gLoad.canvasElems[i];
...
(function () {
    function receiveMessage(event) {
        ...
        switch(message.type) {
          case "SetGame":
            gLoad.initialize(message.loc, message.canvasId);
            break;
        }
    }
...
// Change the gtrl
gCtrl.fireEvent = function (element, eventName, oEvent) {
  if (gLoad.canvasElems.length == 0) {
    document.body.fireEvent('on' + eventName, oEvent);
    return;
  }
  gLoad.canvasElems[0].fireEvent('on' + eventName, oEvent);
  oEvent.bubbles = false;
  for (var i = 1; i < gLoad.canvasElems.length; i++) {
    gLoad.canvasElems[i].fireEvent('on' + eventName, oEvent);
  }
}
gCtrl.simulateEvent = function (element, oEvent) {
  if (gLoad.canvasElems.length == 0) {
    document.body.dispatchEvent(oEvent);
    return;
  }
  gLoad.canvasElems[0].dispatchEvent(oEvent);
  oEvent.bubbles = false;
  for (var i = 1; i < gLoad.canvasElems.length; i++) {
    gLoad.canvasElems[i].dispatchEvent(oEvent);
  }
}